...
count_emails <- function(emails_sum, ADDRESS_TYPE) {
  x <- emails_sum[[ADDRESS_TYPE]] |>
    table() |>
    sort(decreasing = TRUE)
  y <- names(x)
  z <- names(x) |>
    lapply(function(address) {
      emails_sum[[paste0(ADDRESS_TYPE, "_name")]][which(emails_sum[[ADDRESS_TYPE]] == address)][1]
    }) |>
    as.character()
  return(data.frame(
    count = as.integer(x),
...
choose_emails_to_delete_from <- function(outlook, address, n = 2000, individual_choice = FALSE, ask = TRUE) {
  ...
  if (choice_from %in% c(1, 3)) {
    ...
    if (length(emails_from) == 0) {
      return(message("No emails from '", address, "'"))
    }
    ...
    for (i in seq_along(emails_from)) { # i <- seq_along(emails_from) |> sample(1)
      ...
      if (choice_from == 1) {
        try({
          emails_from[[i]]$delete(confirm = FALSE)
        })
